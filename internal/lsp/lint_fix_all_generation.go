...
// speculativeLintEnvironment freezes every external input used to rebuild
// isolated fix-all generations.
...
func (s *speculativeGenerationProvider) AcquireGeneration(
	ctx context.Context,
	...
	if s.acquire == nil {
		return linter.Generation{}, nil, fmt.Errorf("LSP fix-all generation provider for %s is not configured", s.uri)
	}
	if err := ctx.Err(); err != nil {
		return linter.Generation{}, nil, err
	}
	...
	if len(files) > 0 {
		if len(files) != 1 || files[0].Path != s.targetPath {
			return linter.Generation{}, nil, fmt.Errorf("invalid LSP fix-all snapshot for %s", s.uri)
		}
		content = files[0].Text
	}
	return s.acquire(ctx, content, s.snapshot, s.environment)
}
