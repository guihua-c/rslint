...
// a request-local overlay and rebuilds the exact Program/target generation. It
// never owns fix rounds or mutates disk.
...
func (p *cliGenerationProvider) AcquireGeneration(
	...
	if p.rebuild == nil {
		return linter.Generation{}, nil, errors.New("rebuild Programs from in-memory snapshot: provider is not configured")
	}
	binding, fsys, err := p.rebuild(ctx, snapshot)
	if err != nil {
		return linter.Generation{}, nil, fmt.Errorf("rebuild Programs from in-memory snapshot: %w", err)
	}
	if len(binding.Programs) == 0 {
		return linter.Generation{}, nil, errors.New("rebuild Programs from in-memory snapshot: no Program returned")
	}
